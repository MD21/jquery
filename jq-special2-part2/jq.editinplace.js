...
$.fn.editInPlace.defaults = {
	...
	save_button:		'<input type = image src = "images/save.png" class="inplace_save" width =20px height = 20px/>', // string: image button tag to use as “Save” button
	cancel_button:		'<input type = image src = "images/delete.png" class="inplace_cancel"width =20px height = 20px/>', // string: image button tag to use as “Cancel” button
	...
	default_text:		"Add a new Todo here...", // string: text to show up if the element that has this functionality is empty
	...
	text_size:			"30px", // integer: set cols attribute of text input, if field_type is set to text. Use CSS if possible though
...
$.extend(InlineEditor.prototype, {
	...
	connectOpeningEvents: function() {
		...
		this.dom
			...
			*/
                 .bind('click', function(anEvent){$(this).css("border","");that.openEditor(anEvent,this); });
	},
	...
	openEditor: function(anEvent,that) {
		if ( ! this.shouldOpenEditor(anEvent))
			return;
		
		this.disconnectOpeningEvents();
		this.removeHoverEffect();
		this.removeInsertedDefaultTextIfNeccessary(that);
	...
	removeInsertedDefaultTextIfNeccessary: function(that) {
		if ( ! this.didInsertDefaultText
			|| this.dom.html() !== this.settings.default_text)
			return;
		console.log($(that).text());
		this.dom.html($(that).text());
		this.didInsertDefaultText = false;
	},
	...
	connectClosingEventsToEditor: function() {
		...
		var button = this.dom.siblings("#img_todo");

		form.find(".inplace_field").focus().select();
		button.find(".inplace_cancel").click(cancelEditorAction);
		button.find(".inplace_save").click(saveEditorAction);
	...
	markEditorAsInactive: function() {
                $(this.dom).css("border","1px solid");
                $("#img_todo img.add").replaceWith("<img src = 'images/edit.png' class = edit>");
